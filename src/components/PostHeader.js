...
const PostHeader = ({ isTeaser, id, title, published, flow, hubs }) => (
  <div className="post__header">
    ...
    <h2 className="post__title">
      { isTeaser ? <div><a href={`/#/flows/${flow.id}/`} className="post__flow" onClick={ga.bind(void 0, 'flow', 'feed page', flow.name)}>{flow.name}</a><span className="post__title-arrow"> → </span><a href={`/#/post/${id}/`} className="post__title_link">{title}</a></div> : <span>{title}</span> }
    </h2>
    <div className="hubs">{/* TODO добавить запятые между элементами списка посредством CSS */}
      ...
      {hubs.map(hub =>
        <a key={hub.id} href={`/#/hub/${hub.id}/`} className={`hub${hub.isSubscribed ? ' subscribed' : ''}`} title={hub.isSubscribed ? 'Вы подписаны на этот хаб' : 'Вы не подписаны на этот хаб'} onClick={ga.bind(void 0, 'hub', 'feed page', hub.name)}>{hub.name}</a>
      )}
    </div>
  </div>
)
